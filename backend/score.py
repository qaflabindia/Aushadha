...
# Clinical intelligence is now autonomously inferred by the Intelligence Engine (LLM) in src/llm.py.
# Deterministic Python rules are removed in favor of semantic inference.
...
async def graph_query(
    ...
    """Query the graph for results based on document names. Translates labels when language is not English."""
    try:
        start = time.time()
...
async def extract_clinical_intelligence(
    ...
    try:
        ...
        # We now trust the LLM's inference for bp_category, status, and red_flag_list.
        db = SessionLocal()
        try:
            ...
            # Add Lifestyle Factors/Suspicions
...
@app.get("/ehr_data/{file_name}")
async def get_ehr_data(file_name: str):
...
@app.put("/ehr_data/{case_id}")
async def update_ehr_data(case_id: str, request: Request):
    ...
    try:
        ...
        try:
            ...
            if visit:
                ...
                # We re-evaluate the entire clinical context using the LLM for any updates.
                # Prepare text snapshot for inference
                ...
                if inference_result:
                    ...
                    # Sync statuses back to vitals/symptoms if inferred differently
...
async def sync_ehr_to_kg(
    ...
    try:
        ...
        try:
            ...
            # Simple Cypher push (Modular orchestration)
            cypher = """
            ...
            MERGE (p)-[:HAS_VISIT]->(v)
            WITH v
            // Remove existing vitals/symptoms/lifestyle in graph for this visit before re-adding
