...
const SecretVaultModal: React.FC<SecretVaultModalProps> = ({ open, onClose }) => {
    ...
    return (
        <Dialog isOpen={open} onClose={onClose} size="small" className="ndl-dialog">
            <Dialog.Header>
                <div className="flex items-center gap-2">
                    <LockClosedIconOutline className="n-size-token-7 text-[#D4AF37]" />
                    <Typography variant="h3" className="!text-[#D4AF37] font-bold">Secret Vault</Typography>
                </div>
            </Dialog.Header>
            <Dialog.Content className="flex flex-col gap-6">
                ...
                <div className="mt-2 text-right">
                    <Button onClick={handleSave} className="ndl-button-primary">Save Secret</Button>
                </div>

                <div className="mt-4 pt-4 border-t border-[#D4AF37]/20">
                    <Typography variant="subheading-medium" className="!text-[#D4AF37] mb-3 uppercase tracking-wider text-xs">Configured Secrets</Typography>
                    <div className="flex flex-wrap gap-2 mt-2">
                        {existingSecrets.length > 0 ? (
                            existingSecrets.map((key) => (
                                <div key={key} className="luxury-tag px-3 py-1.5 rounded-full flex items-center gap-2 border border-[#D4AF37]/30 bg-[#D4AF37]/10">
                                    <Typography variant="body-small" className="font-bold tracking-tight">{key}</Typography>
                                    <LockClosedIconOutline className="n-size-token-4 text-[#D4AF37]" />
                                </div>
                            ))
                        ) : (
                            <Typography variant="body-small" className="text-gray-500 italic">No secrets configured yet.</Typography>
                        )}
                    </div>
                </div>
            </Dialog.Content>
            <Dialog.Actions>
                <Button onClick={onClose} fill="text" className="!text-gray-400 hover:!text-white">Close</Button>
            </Dialog.Actions>
        </Dialog>
    );
};
